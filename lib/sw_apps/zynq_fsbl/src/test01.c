...
#define SoftReset()		Xil_Out32(XPS_SYS_CTRL_BASEADDR + 0x200, 0x00000001)
#define XDCFG_MULTIBOOT_ADDR_REG		XPS_DEV_CFG_APB_BASEADDR + 0x2C
...
int ddr_test(void)
{
    ...
    XGpioPs GPIO_driver;				//Global Driver Instance PS
  ...
  case 0x54:
	...
	ddriob_get();
	printMemTestHeader();
	measure_write_eye(imin, imax, istep, &gresult[0], 1.0, testsize_scl, fast, fix_center, 1);
	ddrc_init();
